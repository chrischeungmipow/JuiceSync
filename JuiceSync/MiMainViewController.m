...
@interface MiMainViewController ()
{
    ...
    UIAlertView *connectEstablishAlert;
...
@synthesize capacityPercent,chargingState,detailItem,deviceName,myPeriphMain,tempEmpty,tempLabel,outRangeLabel,tempAlertBtn,batteryAlertBtn,rangeAlertBtn,batteryAlertLabel,tempAlertLabel,rangeAlertLabel,batteryLife;
...
- (void)viewDidLoad
{
    ...
    tempBody = [[UIImageView alloc] initWithFrame:CGRectMake(38, 325, 200, 12)];
    tempBody.image = [UIImage imageNamed:@"temp_body_blue.png"];
    [[self.view viewWithTag:0] addSubview:tempBody];
    
    self.tempLiquidView = [[MiTempLiquidView alloc] initWithFrame:CGRectMake(38, 325, 200,12)];
    //[self.liquidView initLiquidView:CGRectMake(52, 48, ViewWidth-55*2,ViewWidth-55*2)];
    self.tempLiquidView.backgroundColor = [UIColor clearColor];
    [self.tempLiquidView setProgress:0.0/100 animated:YES];
    [self.view addSubview:self.tempLiquidView];
    //[self.view bringSubviewToFront:self.tempLiquidView];
    [self.view bringSubviewToFront:tempEmpty];
    ...
    self.liquidView = [[MiLiquidView alloc] initWithFrame:CGRectMake(58, 50, ViewWidth-58*2,ViewWidth-58*2)];
    //[self.liquidView initLiquidView:CGRectMake(52, 48, ViewWidth-55*2,ViewWidth-55*2)];
   ...
   /* if (detailItem)
    ...
    if (self.detailItem)
    {
        self.myPeriphMain = (CBPeripheral*) self.detailItem;
        self.title =self.myPeriphMain.name;
        [instance.manager connectPeripheral:self.myPeriphMain
                                    options:[NSDictionary dictionaryWithObject:[NSNumber numberWithBool:YES] forKey:CBConnectPeripheralOptionNotifyOnDisconnectionKey]];
        [[NSUserDefaults standardUserDefaults]setObject:[self GetUUID] forKey:@"lastConnectUUID"];
        
    }else{
        //in case that peripheral has been connected, and app is killed.
        
        NSArray *connnectedPeriph = [instance.manager retrieveConnectedPeripheralsWithServices:[NSArray arrayWithObjects:[CBUUID UUIDWithString:BATTERY_RATE_SERVICE_UUID],
                                                                                                [CBUUID UUIDWithString:BATTERY_CAPACITY_SERVICE_UUID], [CBUUID UUIDWithString:BATTERY_RATE_SERVICE_UUID],nil]];
        for (CBPeripheral *myPeriph in connnectedPeriph){
            
            NSString *uuid = [self GetUUID:myPeriph];
            if([[[NSUserDefaults standardUserDefaults] objectForKey:@"lastConnectUUID"] isEqualToString:uuid])
            {
                NSLog(@"myPeriphname %@", myPeriph.name);
                //[instance.holdPeripherals addObject:myPeriph];????????????????????????????
                self.myPeriphMain = myPeriph;
            }
        }
        if(self.myPeriphMain){
            [instance.manager cancelPeripheralConnection:self.myPeriphMain];
            
            [self performSelector:@selector(notifyDelay) withObject:nil afterDelay:3];
        }
        else{
            [self toCannotEstablish];
        }
    }

    //get capacity and temperature right now
    CBCharacteristic *myCharacterCapacity = [instance.fastCharsCBUUID objectForKey:[CBUUID UUIDWithString:BATTERY_CAPACITY_CHAR_UUID]];
    
    if (myCharacterCapacity) {
        instance = [MiBlueToothGlobal shared];
        /*NSMutableDictionary *connectedPeriphs  = [MiBlueToothGlobal getStoredPeripherals];
        for (CBPeripheral *myPeriph in connectedPeriphs) {
            
        }*/
        [instance.testPeripheral readValueForCharacteristic:myCharacterCapacity];
    }
    
    CBCharacteristic *myCharacterTemper = [instance.fastCharsCBUUID objectForKey:[CBUUID UUIDWithString:BATTERY_STATE_CHAR_UUID]];
    
    if (myCharacterTemper) {
        instance = [MiBlueToothGlobal shared];
        [instance.testPeripheral readValueForCharacteristic:myCharacterTemper];
    }
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"AllBackToCase" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(allBackToCase:)
                                                 name:@"AllBackToCase"
                                               object:nil];
...
-(void)showTempChart:(uint16_t)temperatureCelsius{
    ...
    if (IS_IPHONE_5) {
        ...
        if (temperatureCelsius < TEMP_THRESHOLD){
            tempBody.image = [UIImage imageNamed:@"temp_body_blue.png"];
            //tempBody.frame = CGRectMake(38, 325, temperatureCelsius*2.00, 12);
            [self.tempLiquidView setProgress:(float)temperatureCelsius/100 animated:YES];
...
- (void)receivedPeriphConnectedNotification:(NSNotification *) notification {
    ...
    if(reconnPeriph){
        ...
        //[[NSUserDefaults standardUserDefaults]setObject:nil forKey:@"lastConnectUUID"];
        myPeriphMain = reconnPeriph;
        [instance.manager connectPeripheral:myPeriphMain
                                    options:[NSDictionary dictionaryWithObject:[NSNumber numberWithBool:YES] forKey:CBConnectPeripheralOptionNotifyOnDisconnectionKey]];
        //grayview.hidden = YES;
        NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
...
- (NSString *)GetUUID
{
    CFUUIDRef theUUID = [self.myPeriphMain UUID];
    if (theUUID == 0x0) {
        //NSLog(@"Yes");
        return nil;
    }else{
...
-(void)notifyDelay{
    ...
    [instance.manager connectPeripheral:self.myPeriphMain options:[NSDictionary dictionaryWithObject:[NSNumber numberWithBool:YES] forKey:CBConnectPeripheralOptionNotifyOnDisconnectionKey]];
    //[instance restartToAddFastCharsCBUUID:self.myPeriph];
    NSLog(@"is connected %d", [self.myPeriphMain state]);
    NSLog(@"%@",self.myPeriphMain.name);
    
    CBCharacteristic *myCharacter = [instance.fastCharsCBUUID objectForKey:[CBUUID UUIDWithString:BATTERY_CAPACITY_CHAR_UUID]];
    if (myCharacter) {
        [self.myPeriphMain setNotifyValue:YES forCharacteristic:myCharacter];
        //[instance.testPeripheral readValueForCharacteristic:myCharacter];
    }
    
}

-(void)toCannotEstablish{
    //myPeriph = (CBPeripheral*) self.detailItem;
    //if (!self.myPeriph.isConnected) {
    
    connectEstablishAlert =  [[UIAlertView alloc] initWithTitle: NSLocalizedString(@"Prompt", @"")
                                                        message: NSLocalizedString(@"Promptmsg", @"")
                                                       delegate:self
...
-(void)backToMain{
    ...
    [instance.manager cancelPeripheralConnection:myPeriphMain];
    NSLog(@"cancel %@", myPeriphMain.name);
...
- (void)allBackToCase:(NSNotification *) notification{
